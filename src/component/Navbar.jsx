...
export default function Navbar() {
    let Links = [
        { name: "Home", link: "/" },
        { name: "Student Info", link: "/studentInfo" },
        { name: "Academic Details", link: "/studentAcademics" },
        { name: "Blogs", link: "/" },
    ...
    return ( 
        <div className='shadow-md w-full fixed top-0 left-0 fixed'>
            <div className='md:flex items-center justify-between bg-[#004e89] py-2 md:px-10 px-5'>
                {/* Logo Section */}
                ...
                <ul className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static bg-[#004e89] md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in ${open ? 'top-20' : 'top-[-490px]'}`}>
                    {Links.map((link) => (
                        <li key={link.name} className='md:ml-8 text-lg md:my-0 my-2'>                        
...
const Button = (props) => {
    return (
        <button className='bg-white text-[#004e89] cursor-pointer font-[Poppins] font-semibold py-1.5 px-3 rounded md:ml-8 hover:bg-[#adb5bd] duration-300'>
            {props.children}
        </button>
    );
};
